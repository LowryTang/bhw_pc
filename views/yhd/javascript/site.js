//关闭product购物车弹出的div
function closeCartDiv()
{
	$('#product_myCart').hide('slow');
	$('.submit_join').removeAttr('disabled','');
}

//商品移除购物车
function removeCart(urlVal,goods_id,type)
{
	var goods_id = parseInt(goods_id);

	$.getJSON(urlVal,{goods_id:goods_id,type:type},function(content){
		if(content.isError == false)
		{
			$('[name="mycart_count"]').html(content.data['count']);
			$('[name="mycart_sum"]').html(content.data['sum']);
		}
		else
		{
			alert(content.message);
		}
	});
}

//添加收藏夹
function favorite_add_ajax(urlVal,goods_id,obj)
{
	$.getJSON(urlVal,{goods_id:goods_id,nocache:((new Date()).valueOf())},function(content){
		if(content.isError == false)
		{
			obj.value = content.message;
		}
		else
		{
			alert(content.message);
		}
	});
}

//寄存购物车[ajax]
function deposit_ajax(urlVal)
{
	$.getJSON(urlVal,{is_ajax:'1'},function(content){
		if(content.isError == false)
		{
			alert(content.message);
		}
		else
		{
			alert(content.message);
		}
	});
}

//购物车展示
function showCart(urlVal)
{
	$.getJSON(urlVal,{sign:Math.random()},function(content)
	{
		var cartTemplate = template.render('cartTemplete',{'goodsData':content.data,'goodsCount':content.count,'goodsSum':content.sum});
		$('#div_mycart').html(cartTemplate);
		$('#div_mycart').show();
	});
}

//自动完成
function autoComplete(ajaxUrl,linkUrl,minLimit)
{
	var minLimit = minLimit ? parseInt(minLimit) : 2;
	var maxLimit = 10;
	var keywords = $.trim($('input:text[name="word"]').val());

	//输入的字数通过规定字数
	if(keywords.length >= minLimit && keywords.length <= maxLimit)
	{
		$.getJSON(ajaxUrl,{word:keywords},function(content){

			//清空自动完成数据
			$('.auto_list').empty();

			if(content.isError == false)
			{
				for(var i=0; i < content.data.length; i++)
				{
					var searchUrl = linkUrl.replace('@word@',content.data[i].word);
					$('.auto_list').append('<li onclick="event_link(\''+searchUrl+'\')" style="cursor:pointer"><a href="javascript:void(0)">'+content.data[i].word+'</a>约'+content.data[i].goods_nums+'个结果</li>');
					//鼠标经过效果
					$('.auto_list li').bind("mouseover",
						function()
						{
							$(this).addClass('hover');
						}
					);
					$('.auto_list li').bind("mouseout",
						function()
						{
							$(this).removeClass('hover');
						}
					);
				}
				$('.auto_list').show();
			}
			else
			{
				$('.auto_list').hide();
			}
		});
	}
	else
	{
		$('.auto_list').hide();
	}
}

//输入框
function checkInput(para,textVal)
{
	var inputObj = (typeof(para) == 'object') ? para : $('input:text[name="'+para+'"]');

	if(inputObj.val() == '')
	{
		inputObj.val(textVal);
	}
	else if(inputObj.val() == textVal)
	{
		inputObj.val('');
	}
}

//dom载入成功后开始操作
jQuery(function()
{

	var $category = $('#on li:gt(5)');
	$category.hide();
	var $toggleBtn = $('.multiple_choice a');
	$toggleBtn.toggle(function(){
		$category.show();
		$(this).find('span').removeClass('hide');	
		$("#ma").html('收起');
	},function(){
		$category.hide();
		$(this).find('span').addClass('show');
		$("#ma").html('更多');
	});

	
	var $showgory = $('#other');
	$showgory.hide();
	var $toggleA = $('.more_btn a');
	$toggleA.toggle(function(){
		$showgory.show();
		$(this).find('span').removeClass('hide').addClass('show').text('更多类目');	
	},function(){
		$showgory.hide();
		$(this).find('span').removeClass('show').addClass('hide').text('更多类目');
	});
	

	$('#mobie_list').hover(
		function(){
			$('.hd_mobile_show').show();
		},
		function(){
			allsortLateCall.stop();
			$('.hd_mobile_show').hide();
		}
	);
		
	$('#help_list').hover(
		function(){
			$('.hd_menu_list').show();
		},
		function(){
			allsortLateCall.stop();
			$('.hd_menu_list').hide();
		}
	);
	
	var allsortLateCall = new lateCall(200,function(){$('.hd_allsort_out').show();});
	//商品分类
	$('#allSortOuterbox').hover(
		function(){
			allsortLateCall.start();
		},
		function(){
			allsortLateCall.stop();
			$('.hd_allsort_out').hide();
		}
	);
	$('.hd_allsort_out ul li').each(
		function(i)
		{
			$(this).hover(
				function(){
					$(this).addClass('hover');
					$('.hd_show_sort:eq('+i+')').show();
				},
				function(){
					$(this).removeClass('hover');
					$('.hd_show_sort:eq('+i+')').hide();
				}
			);
		}
	);

	//排行,浏览记录的图片
	$('#ranklist li').hover(
		function(){
			$(this).addClass('current');
		},
		function(){
			$(this).removeClass('current');
		}
	);

	//自动完成input框 事件绑定
	var tmpObj = $('input:text[name="word"]');
	var defaultText = tmpObj.val();
	tmpObj.bind({
		focus:function(){checkInput($(this),defaultText);},
		blur :function(){checkInput($(this),defaultText);}
	});
	
	bann_pic();
	
});


//图片左右滚动
function bann_pic() {
	var sWidth = $("#middle_bann_img").height(); 
	var len = $("#middle_bann_img ul li").length; 
	var index = 0;
	var picTimer;
	
	var btn = "<div class='btnBg'></div><div class='btn'>";
	for(var i=0; i < len; i++) {
		btn += "<span></span>";
	}
	btn += "</div><div class='preNext pre'></div><div class='preNext next'></div>";
	$("#middle_bann_img").append(btn);
	$("#middle_bann_img .btnBg").css("opacity",0.5);


	$("#middle_bann_img .btn span").css("opacity",0.4).mouseenter(function() {
		index = $("#middle_bann_img .btn span").index(this);
		showPics(index);
	}).eq(0).trigger("mouseenter");
    
	
	$("#middle_bann_img ul").css("height",sWidth * (len));
	

	$("#middle_bann_img").hover(function() {
		clearInterval(picTimer);
	},function() {
		picTimer = setInterval(function() {
			showPics(index);
			index++;
			if(index == len) {index = 0;}
		},4000); 
	}).trigger("mouseleave");
	
	
	function showPics(index) {
		var nowLeft = -index*sWidth; 
		$("#middle_bann_img ul").stop(true,false).animate({"top":nowLeft},300);
		$("#middle_bann_img .btn span").stop(true,false).animate({"opacity":"0.4"},300).eq(index).stop(true,false).animate({"opacity":"1"},300);
	}

};


$("#back_top").hide(); 
$("#sc_wap em").hide(); 
$("#sc_box em").hide(); 
$(function () { 
	$(window).scroll(function(){ 
		if ($(window).scrollTop()>100){ 
			$("#back_top").fadeIn(1500); 
		}else{ 
			$("#back_top").fadeOut(1500); 
		} 
	});
	$(".goTopBtn").click(function(){ 
		$('body,html').animate({scrollTop:0},500); 
		return false; 
	});
	
	$('#sc_wap').hover(
		function(){
			$("#sc_wap em").show(); 
		},function(){
			$("#sc_wap em").hide(); 
		}
	);
	
	$('#sc_box').hover(
		function(){
			$("#sc_box em").show(); 
		},function(){
			$("#sc_box em").hide(); 
		}
	);
	
	$('.hd_weixin').hover(
		function(){
			$(".hd_weixin i").show(); 
		},function(){
			$(".hd_weixin i").hide(); 
		}
	);
	
});

//teb切换效果
function cutover(id,etype){
	var tabContainer = $("."+id).find(".tabContainer");
	var panelContainer = $("."+id).find(".panelContainer");	
	tabContainer.find("ul>li").each(function(i,e){
		if (etype=="1"){
			$(e).hover(function(){
				tabContainer.find("ul>li").removeClass("on");
				$(e).addClass("on");
				panelContainer.children("div").hide();
				panelContainer.children("div").eq(i).show();
			});
		}
	});
};


//图片左右滚动
function roll_pic(flash_box,time){
	var sWidth = $("#sxSliderIndexAd"+flash_box).width(); 
	var len = $("#sxSliderIndexAd"+flash_box+" ul li").length; 
	var index = 0;
	var picTimer;
	
	var btn = "<div class='btnBg'></div><div class='btn'>";
	for(var i=0; i < len; i++) {
		btn += "<span></span>";
	}
	btn += "</div><div class='preNext pre'></div><div class='preNext next'></div>";
	$("#sxSliderIndexAd"+flash_box).append(btn);
	$("#sxSliderIndexAd"+flash_box+" .btnBg").css("opacity",0.5);


	$("#sxSliderIndexAd"+flash_box+" .btn span").css("opacity",0.4).mouseenter(function() {
		index = $("#sxSliderIndexAd"+flash_box+" .btn span").index(this);
		showPics(index);
	}).eq(0).trigger("mouseenter");
    
	
	$("#sxSliderIndexAd"+flash_box+" ul").css("width",sWidth * (len));
	

	$("#sxSliderIndexAd"+flash_box).hover(function() {
		clearInterval(picTimer);
	},function() {
		picTimer = setInterval(function() {
			showPics(index);
			index++;
			if(index == len) {index = 0;}
		},4000); 
	}).trigger("mouseleave");
	
	
	function showPics(index) {
		var nowLeft = -index*sWidth; 
		$("#sxSliderIndexAd"+flash_box+" ul").stop(true,false).animate({"left":nowLeft},300);
		$("#sxSliderIndexAd"+flash_box+" .btn span").stop(true,false).animate({"opacity":"0.4"},300).eq(index).stop(true,false).animate({"opacity":"1"},300);
	}

};